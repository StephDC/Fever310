...
```{r}
##MAKE THE FUCKING WEEKS SINCE 1990 JAN 1 
train_features$week_start_date=strptime(train_features$week_start_date, format="%Y-%m-%d")
startDate = ISOdate(1990,01,01)
numWeeks <- c()
for(i in length(train_features))
  numWeeks[i] <- difftime(train_features$week_start_date[i], startDate, units = "weeks")
```
